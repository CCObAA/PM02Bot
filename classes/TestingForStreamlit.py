...
class TestStrat:
    ...
    def main(symbols):
        ...
        for symbol in symbols:
            ...
            df.Close = df['Close']
            TestStrat.applyindicators(df)
            TestStrat.conditions(df)
            trades = TestStrat.matchtrades(df)
        ...
        # (sorted_df.profit + 1).cumprod().plot() #Какой-то график
        ...
        # (realtrades.profit + 1).cumprod().plot() # График профита без комисси

        realtrades['Net Profit'] = realtrades['profit'] - 0.005

        # (realtrades['Net Profit'] + 1).cumprod().plot() # График профита с комиссией
